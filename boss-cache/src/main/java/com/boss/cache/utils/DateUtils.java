...
public class DateUtils {
    ...
    public static String DayAfterTime2(Integer days) {
        Date now = new Date();
        Calendar cal = Calendar.getInstance();
        cal.setTime(now);
        cal.add(Calendar.DAY_OF_MONTH, days);
        Date afterTime = cal.getTime();
        String ttl = DateUtils.format(afterTime, "yyyy-MM-dd");
        return ttl;
    }

    public static String formatFloatData(Float data) {
        if (data == null) {
            return "";
        }
    ...
    public static Float floatConvert(Float data) {
        if (data == null) {
            return 0f;
        }
        DecimalFormat fnum = new DecimalFormat("##0.00");
        return Float.valueOf(fnum.format(data));
    }
